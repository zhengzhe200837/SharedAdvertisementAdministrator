...
public class ChartUtils {
    ...
    public static LineChart initChart(LineChart chart) {
        ...
        xAxis.setGridColor(Color.parseColor("#30777777"));
        // 设置x轴数据偏移量
        xAxis.setYOffset(-12);
    ...
    public static void notifyDataSetChanged(LineChart chart, List<Entry> values,
        ...
        chart.getXAxis().setValueFormatter(new IAxisValueFormatter() {
            ...
            public String getFormattedValue(float value, AxisBase axis) {
                android.util.Log.d("zz", "value = " + value);
                android.util.Log.d("zz", "xValuesProcess = " + xValuesProcess(valueType)[(int) value]);
                return xValuesProcess(valueType)[(int) value];  //返回x轴数据
            }
        });
//        zhengzhe start
...
//            public String getFormattedValue(float value, AxisBase axis) {
//                android.util.Log.d("zz", "value = " + value);
//                android.util.Log.d("zz", "value + i = " + i);
//                return String.valueOf(i++);  //返回Y轴数据
//            }
//        });
//        zhengzhe end
    ...
    private static String[] xValuesProcess(int valueType) {
        ...
        if (valueType == dayValue) { // 今日
            ...
            for (int i = 6; i >= 0; i--) {
                dayValues[i] = TimeUtils.dateToString(currentTime, TimeUtils.DATE_HOUR_MIN);
            ...
            long currentTime = System.currentTimeMillis();
            for (int i = 6; i >= 0; i--) {
                monthValues[i] = TimeUtils.dateToString(currentTime, TimeUtils.DATE_MONTH_DAY);
